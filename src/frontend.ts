...
function minRulerWidth(ruler?: HTMLElement | null): number {
  return readCssNumber('--lrr-min-width', MIN_RULER_WIDTH, ruler)
}

function maxEdgePanelWidth(ruler?: HTMLElement | null): number {
...
function findInputAnchor(): HTMLElement | null {
  ...
  const candidates = Array.from(
    document.querySelectorAll(
      'textarea, [contenteditable="true"], [contenteditable=""], [role="textbox"], input[type="text"], input:not([type]), input[placeholder*="message" i], textarea[placeholder*="message" i]',
    ),
  ).filter(isVisibleElement)
...
function isProbablyEdgePanel(
  ...
  if (nearlyFullscreen || tooWideForPanel || !nearOwnSide) return false
...
function horizontalInsets(
  ...
  if (left + right > maxTotalInset) {
    ...
    if (left > right) left = Math.max(baseInset, left - overflow)
    else right = Math.max(baseInset, right - overflow)
  }

  return { left: Math.round(left), right: Math.round(right) }
}

function shouldYieldToAppUi(ruler: HTMLElement, inputAnchor: HTMLElement | null): boolean {
  ...
  if (isMobileViewport(ruler) && !mobileYieldUi(ruler)) return false
...
export function setup(ctx: SpindleFrontendContext) {
  ...
  let unbindInputAction: (() => void) | null = null
  ...
  try {
    const registerInputBarAction = (ctx.ui as any)?.registerInputBarAction
  ...
  try {
    const uiEvents = (ctx.ui as any)?.events
    if (typeof uiEvents?.onDrawerChange === 'function') {
      unbindDrawer = uiEvents.onDrawerChange((state: any) => {
        drawerOpen = Boolean(state?.open)
        scheduleSync()
      })
    }
    if (typeof uiEvents?.onSettingsChange === 'function') {
      unbindSettings = uiEvents.onSettingsChange((state: any) => {
